...
int main (int argc, char **argv)
{
	...
	if (argc < 2) {
		...
		printf ("File lc.dat can have 2 columns (phase and flux),\n");
	...
	for (i = 1; i < argc; i++) {
        ...
        if (strcmp (argv[i], "-c") == 0) {
            ...
            COLIDX[0] = -1; COLIDX[1] = -1; COLIDX[2] = -1;
	...
	while (!feof (in)) {
        ...
        if (colno <= COLIDX[0] || colno <= COLIDX[1] || colno <= COLIDX[2]) {
            printf ("too few columns in the input file, aborting.\n");
            exit(0);
        }
        
        data[i].x = cols[COLIDX[0]];
        data[i].y = cols[COLIDX[1]];
        data[i].z = 1e-3/(COLIDX[2] == -1 ? 1.0 : cols[COLIDX[2]])/(COLIDX[2] == -1 ? 1.0 : cols[COLIDX[2]]);
